defmodule WaziBetWeb.UserAuth do
  ...
  @remember_me_cookie "_wazi_bet_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      WaziBetWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      WaziBetWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule WaziBetWeb.PageLive do
        use WaziBetWeb, :live_view

        on_mount {WaziBetWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{WaziBetWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:mount_categories, _params, _session, socket) do
  ...
  def on_mount({:require_any_permission, permissions}, _params, session, socket)
  ...
  defp mount_categories(socket) do
    Phoenix.Component.assign(socket, :categories, Sport.list_categories())
  end
  ...
  defp redirect_path(%Accounts.User{} = user) do
    admin_permissions = [
      "view-users",
  ...
  @doc """
  Plug for routes that should not be accessible to authenticated users.
  ...
  @doc """
  Plug for routes that require a specific permission.

  ## Examples

      plug WaziBetWeb.UserAuth, :require_permission
      # Then in router:
      plug WaziBetWeb.UserAuth, permission: "view-games"

      # Or for multiple permissions (any):
      plug WaziBetWeb.UserAuth, permissions: ["view-games", "place-bets"]
  """
